...
const AccountProfile = () => {
    ...
    if (!profile) return <div className="bg-error/10 border border-error/30 text-error p-6 rounded-overlay text-center">{t('PROFILE.LOAD_FAILED')}</div>;
                   ...
                   <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 relative z-10">
                      ...
                      <div className="text-left md:text-right">
                         ...
                         <div className="inline-flex items-center gap-2">
                             {profile.role === 'admin'
                               ? <span className="bg-fuchsia-500/20 text-fuchsia-400 text-sm px-3 py-1 rounded-control font-medium border border-fuchsia-500/30">{t('PROFILE.ROLE_ADMIN')}</span>
                               : <span className="bg-surface-container-high text-on-surface-variant text-sm px-3 py-1 rounded-control font-medium border border-outline-variant">{t('PROFILE.ROLE_USER')}</span>
                             }
                         </div>
                      </div>
                   </div>
