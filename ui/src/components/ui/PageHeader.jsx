 ...
 * - icon 容器 11×11 rounded-control（更"app icon"感而非 chip）
...
const PageHeader = ({ title, sub, actions, icon: Icon, children, className = '' }) => (
  <header className={`flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 ${className}`}>
    <div className="flex items-start gap-3.5 min-w-0">
      {Icon && (
        <span className="w-11 h-11 rounded-control bg-primary/10 border border-primary/20 flex items-center justify-center shrink-0 text-primary mt-0.5">
