...
func AdminGrantSubscription(c *fiber.Ctx) error {
	...
	// 委派到核心 helper（同样的逻辑被 AdminGrantSubscriptionBatch 复用）。
	result := executeAdminGrantForUser(op, req, qty, reason, c.IP())
...
// grantResult 一次"赠送给单个用户"的结果（用于 single + batch 两条路径共用）。
type grantResult struct {
	UserID      uint                          `json:"user_id"`
	Created     []database.UserSubscription   `json:"created,omitempty"`
	Status      int                           `json:"-"`            // HTTP-like status code, 不进 JSON
	MessageCode string                        `json:"message_code,omitempty"`
	Message     string                        `json:"message,omitempty"`
}

// executeAdminGrantForUser 是赠送订阅的核心实现，跑完一笔（含事务、缓存失效、通知）。
//
// 与 AdminGrantSubscription 的差异：req-level 入参校验（reason 非空、qty 范围等）由 caller
// 完成，本函数只做"加载实体 + 事务 + 通知"。Status / MessageCode 的取值集合与原 handler 完全
// 一致，让 caller（无论是 single endpoint 还是 batch）能直接映射到 HTTP 响应。
//
// 用户反馈"想批量赠送"——之前所有错误处理都通过 c.Status().JSON() 直接写响应，导致只能 1:1 调用。
// 抽到 helper 后 batch endpoint 能聚合 N 个用户的结果，单用户行为不变。
func executeAdminGrantForUser(op *database.User, req adminGrantPayload, qty int, reason string, clientIP string) grantResult {
	// ─── 加载实体 ──────────────────────────────────────────────
	var targetUser database.User
	if err := database.DB.First(&targetUser, req.UserID).Error; err != nil {
		return grantResult{UserID: req.UserID, Status: fiber.StatusNotFound, MessageCode: "ERR_USER_NOT_FOUND", Message: "目标用户不存在"}
	}
	if targetUser.Role != "user" {
		// 防止给另一个 admin 赠送（admin 之间的"礼物"会让审计很复杂；如确需可后续放开）
		return grantResult{UserID: req.UserID, Status: fiber.StatusForbidden, MessageCode: "ERR_TARGET_NOT_USER", Message: "只能赠送给普通用户（role=user）"}
	}
	if targetUser.Status != 1 {
		return grantResult{UserID: req.UserID, Status: fiber.StatusBadRequest, MessageCode: "ERR_TARGET_USER_INACTIVE", Message: "目标用户状态异常（封禁 / 删除），无法赠送"}
	}

	var pkg database.Package
	if err := database.DB.First(&pkg, req.PackageID).Error; err != nil {
		return grantResult{UserID: req.UserID, Status: fiber.StatusNotFound, MessageCode: "ERR_PACKAGE_NOT_FOUND", Message: "套餐不存在"}
	}
	if !pkg.IsEnabled() {
		return grantResult{UserID: req.UserID, Status: fiber.StatusBadRequest, MessageCode: "ERR_PACKAGE_DISABLED", Message: "套餐已禁用，无法赠送"}
	}
	...
	if pkg.PriceAmount < 0 {
		...
		return grantResult{UserID: req.UserID, Status: fiber.StatusInternalServerError, MessageCode: "ERR_PACKAGE_INVALID_NUMERIC", Message: "套餐金额数据损坏"}
	}
	if pkg.BillingPeriodSeconds <= 0 || int64(pkg.BillingPeriodSeconds) > maxBillingPeriodSec {
		...
		return grantResult{UserID: req.UserID, Status: fiber.StatusInternalServerError, MessageCode: "ERR_PACKAGE_INVALID_PERIOD", Message: "套餐 billing_period_seconds 越界"}
	}
	...
	if effectiveMax > 0 {
		...
		if err := database.DB.Model(&database.UserSubscription{}).
			...
			return grantResult{UserID: req.UserID, Status: fiber.StatusInternalServerError, MessageCode: "ERR_DB_QUERY", Message: "DB 查询失败"}
		}
		if int(activeCount)+qty > effectiveMax {
			return grantResult{UserID: req.UserID, Status: fiber.StatusConflict, MessageCode: "ERR_STACK_LIMIT", Message: fmt.Sprintf("目标用户已达该套餐持有上限 %d 份", effectiveMax)}
		}
	}
	...
	if err != nil {
		...
		return grantResult{UserID: req.UserID, Status: fiber.StatusInternalServerError, MessageCode: "ERR_BUILD_SNAPSHOT", Message: "构建套餐快照失败"}
	}
	...
	txErr := database.DB.Transaction(func(tx *gorm.DB) error {
		...
		if err := LogOperationByTx(tx, op.ID, req.UserID, "admin", "GRANT_SUBSCRIPTION", clientIP, string(detailsJSON)); err != nil {
	...
	if txErr != nil {
		if errors.Is(txErr, errStackLimitExceeded) {
			...
			return grantResult{UserID: req.UserID, Status: fiber.StatusConflict, MessageCode: "ERR_STACK_LIMIT", Message: fmt.Sprintf("已达该套餐叠加上限 %d 份（事务内并发检查）", limitForMsg)}
		}
		if errors.Is(txErr, errTargetUserChanged) {
			return grantResult{UserID: req.UserID, Status: fiber.StatusConflict, MessageCode: "ERR_TARGET_USER_CHANGED", Message: "目标用户状态在事务期间发生变化（被封禁 / 角色变更），赠送已回滚"}
		}
		if errors.Is(txErr, errPackageChanged) {
			return grantResult{UserID: req.UserID, Status: fiber.StatusConflict, MessageCode: "ERR_PACKAGE_CHANGED", Message: "套餐状态在事务期间发生变化（被禁用 / invariant 失效），赠送已回滚"}
		}
		...
		if errors.Is(txErr, errPackageInvalidPeriodInTx) {
			...
			return grantResult{UserID: req.UserID, Status: fiber.StatusInternalServerError, MessageCode: "ERR_PACKAGE_INVALID_PERIOD", Message: "套餐 billing_period_seconds 数据损坏（DB 直改 / 并发越界），赠送已回滚"}
		}
		log.Printf("[GRANT] tx failed admin=%d target=%d pkg=%d err=%v", op.ID, req.UserID, pkg.ID, txErr)
		return grantResult{UserID: req.UserID, Status: fiber.StatusInternalServerError, MessageCode: "ERR_DB_TRANSACTION", Message: "数据库事务失败"}
	}
	...
	return grantResult{UserID: req.UserID, Created: created, Status: fiber.StatusOK, MessageCode: "SUCCESS_GRANTED"}
}

// ─── 批量赠送 ────────────────────────────────────────────

// adminGrantBatchPayload 是 /api/admin/subscriptions/grant-batch 的请求体。
// 用户反馈"只能一个一个赠送、搞个批量啊"——这条 endpoint 接受多个 user_ids，
// 内部按 user 各跑一次事务（失败隔离），最终聚合返回成功 / 失败明细。
type adminGrantBatchPayload struct {
...
const grantBatchMaxUsers = 200 // admin 一次最多批量赠送的用户数（防误粘 10k user_ids）

// AdminGrantSubscriptionBatch POST /api/admin/subscriptions/grant-batch
//
// 行为：
//   - 公共字段（package_id / quantity / reason / valid_seconds）只校验一次
//   - user_ids 去重后逐个走 executeAdminGrantForUser，失败的不影响其它人
//   - 返回 { success, data: { successes: [{user_id, sub_ids}], failures: [{user_id, message_code, message}] } }
//   - 即使部分用户失败，整体 HTTP 仍然 200 —— admin UI 自行展示明细
func AdminGrantSubscriptionBatch(c *fiber.Ctx) error {
	op := loadAdminUser(c)
	if op == nil {
		return c.Status(401).JSON(fiber.Map{"success": false, "message_code": "ERR_NO_AUTH"})
	}

	var req adminGrantBatchPayload
	if err := c.BodyParser(&req); err != nil {
		return c.Status(400).JSON(fiber.Map{"success": false, "message_code": "ERR_PARSE_PAYLOAD"})
	}
	if len(req.UserIDs) == 0 || req.PackageID == 0 {
		return c.Status(400).JSON(fiber.Map{"success": false, "message": "user_ids 与 package_id 必填", "message_code": "ERR_INVALID_PARAMS"})
	}
	if len(req.UserIDs) > grantBatchMaxUsers {
		return c.Status(400).JSON(fiber.Map{"success": false, "message": fmt.Sprintf("user_ids 不能超过 %d 个", grantBatchMaxUsers), "message_code": "ERR_BATCH_TOO_LARGE"})
	}
	// 去重 + 拒绝 admin 自己
	...
	if len(uniqIDs) == 0 {
		return c.Status(400).JSON(fiber.Map{"success": false, "message": "user_ids 去重后为空（不能给 admin 自己赠送）", "message_code": "ERR_NO_VALID_USERS"})
	}

	// 公共字段（qty / reason / valid_seconds）—— 与 single endpoint 同一份校验逻辑。
	qty := 1
	if req.Quantity != nil {
		if *req.Quantity < 1 {
			return c.Status(400).JSON(fiber.Map{"success": false, "message": "quantity 必须 ≥ 1", "message_code": "ERR_INVALID_QUANTITY"})
		}
		qty = *req.Quantity
	}
	if qty > grantQuantityMaxCap {
		return c.Status(400).JSON(fiber.Map{"success": false, "message": fmt.Sprintf("quantity 不能超过 %d", grantQuantityMaxCap), "message_code": "ERR_QUANTITY_TOO_LARGE"})
	}
	if req.ValidSeconds != nil {
		if *req.ValidSeconds <= 0 {
			return c.Status(400).JSON(fiber.Map{"success": false, "message": "valid_seconds 必须大于 0", "message_code": "ERR_INVALID_VALID_SECONDS"})
		}
		if *req.ValidSeconds > maxBillingPeriodSec {
			return c.Status(400).JSON(fiber.Map{"success": false, "message": fmt.Sprintf("valid_seconds 不能超过 %d 秒", maxBillingPeriodSec), "message_code": "ERR_VALID_SECONDS_TOO_LARGE"})
		}
	}
	reason := strings.TrimSpace(req.Reason)
	if reason == "" {
		return c.Status(400).JSON(fiber.Map{"success": false, "message": "reason 必填（用于审计 / 用户客服查询）", "message_code": "ERR_REASON_REQUIRED"})
	}
	if runeLen := len([]rune(reason)); runeLen > grantReasonMaxLen {
		return c.Status(400).JSON(fiber.Map{"success": false, "message": fmt.Sprintf("reason 长度不能超过 %d 字符（当前 %d）", grantReasonMaxLen, runeLen), "message_code": "ERR_REASON_TOO_LONG"})
	}
	for _, r := range reason {
		if unicode.IsControl(r) {
			return c.Status(400).JSON(fiber.Map{"success": false, "message": "reason 不能包含控制字符", "message_code": "ERR_REASON_CTRL_CHAR"})
		}
	}

	// 逐用户跑同样的赠送流程；失败不传染其它用户。
	successes := make([]grantResult, 0, len(uniqIDs))
	failures := make([]grantResult, 0)
	clientIP := c.IP()
	for _, uid := range uniqIDs {
		sub := adminGrantPayload{
			UserID:       uid,
		...
		res := executeAdminGrantForUser(op, sub, qty, reason, clientIP)
