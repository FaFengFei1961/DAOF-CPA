...
func defaultModelSeeds() []defaultModelSeed {
	...
	return []defaultModelSeed{
		...
		{ProviderKey: "google", ProviderName: "Google Gemini", ModelID: "gemini-3.5-flash", DisplayName: "Gemini 3.5 Flash", OfficialStatus: "alias_or_unofficial", Category: ModelCategoryText, BillingMode: BillingModeToken, Supported: false, Public: false, DefaultEnabled: false, SourceURL: googlePricing, Notes: "CPA 上游 2026-05-20 新增的 Google 官方 Gemini 3.5 Flash（type=gemini, owned_by=google；context 1M / output 64K；thinking dynamic 128-32K, levels minimal/low/medium/high）。Google 官方 pricing 已查实（ai.google.dev/gemini-api/docs/pricing, 2026-05-20）：Input $1.50 / Output $9.00（含 thinking tokens）/ Cached $0.15 per 1M tokens；无 HighAt 阶梯；non-global region 加价至 $1.65/$9.90。**当前未启用**，admin 启用步骤：在 UI 填上 Token{Input:\"1.5\", Output:\"9\", Cached:\"0.15\"} + 切 Supported=true + DefaultEnabled=true。注意：比 gemini-3-flash-preview（$0.5/$3）贵 3 倍，但比 gemini-3.1-pro-preview（$2/$12）便宜约 40%。"},
