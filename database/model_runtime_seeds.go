...
func defaultModelSeeds() []defaultModelSeed {
	...
	return []defaultModelSeed{
		...
		{ProviderKey: "xai", ProviderName: "xAI", ModelID: "grok-imagine-image", DisplayName: "Grok Imagine Image", OfficialStatus: "official_exact", Category: ModelCategoryImage, BillingMode: BillingModeImage, Supported: true, Public: true, DefaultEnabled: false, SourceURL: xaiPricing, Media: []defaultMediaPrice{{Unit: "image", Direction: "input", Price: "0.02", Notes: "edits 路径输入图按 xAI 官方 per-image rate $0.02 计费（docs.x.ai: edits are billed for both the input image and the generated output image）；实际扣费走 cost_in_usd_ticks，本字段仅 precheck 估算用"}, {Unit: "image", Direction: "output", Resolution: "1K", Price: "0.02"}, {Unit: "image", Direction: "output", Resolution: "2K", Price: "0.02"}}, Notes: "Text-to-image runtime via /v1/images/generations；edits 默认 disabled，admin 在 ChannelModel.AllowedEndpoints 加 /v1/images/edits 启用。"},
		{ProviderKey: "xai", ProviderName: "xAI", ModelID: "grok-imagine-image-quality", DisplayName: "Grok Imagine Image Quality", OfficialStatus: "official_exact", Category: ModelCategoryImage, BillingMode: BillingModeImage, Supported: true, Public: true, DefaultEnabled: false, SourceURL: xaiPricing, Media: []defaultMediaPrice{{Unit: "image", Direction: "input", Price: "0.05", Notes: "edits 路径输入图按 xAI 官方 per-image rate $0.05 计费（docs.x.ai: edits are billed for both the input image and the generated output image）；实际扣费走 cost_in_usd_ticks，本字段仅 precheck 估算用"}, {Unit: "image", Direction: "output", Resolution: "1K", Price: "0.05"}, {Unit: "image", Direction: "output", Resolution: "2K", Price: "0.07"}}, Notes: "Text-to-image runtime via /v1/images/generations；edits 默认 disabled，admin 在 ChannelModel.AllowedEndpoints 加 /v1/images/edits 启用。"},
